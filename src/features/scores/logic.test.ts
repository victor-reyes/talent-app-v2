...
test("First user level is 4.46", () => {
  ...
  const result = calculateAverageLevel(
    (firstUser.frontend + firstUser.backend) / 2,
    (firstUser.charismatic + firstUser.teamwork) / 2,
    (firstUser.design + firstUser.management) / 2
  );
